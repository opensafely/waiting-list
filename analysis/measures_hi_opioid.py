...
hi_opioid_rx = all_opioid_rx.where(
...
hi_opioid_rx.tmp_wait_date = tmp_date + days((hi_opioid_rx.date - rtt_start_date).days)

# Standardise Rx dates relative to RTT end date for post-WL prescribing
hi_opioid_rx.tmp_post_date = tmp_date + days((hi_opioid_rx.date - rtt_end_date).days)

# Standardise Rx dates relative to RTT start date for pre-WL prescribing 
hi_opioid_rx.tmp_pre_date = tmp_date + days((hi_opioid_rx.date - (rtt_start_date - days(182))).days)
...
count_hi_opioid_wait = hi_opioid_rx.where(
                hi_opioid_rx.tmp_wait_date.is_on_or_between(INTERVAL.start_date, INTERVAL.end_date)
            ).count_for_patient()

# Num opioid Rx post waiting list (up to 6 months)
count_hi_opioid_post = hi_opioid_rx.where(
                hi_opioid_rx.tmp_post_date.is_on_or_between(INTERVAL.start_date, INTERVAL.end_date)
            ).count_for_patient()

# Num opioid Rx pre waiting list (up to 6 months)
count_hi_opioid_pre = hi_opioid_rx.where(
                hi_opioid_rx.tmp_pre_date.is_on_or_between(INTERVAL.start_date, INTERVAL.end_date)
            ).count_for_patient()
...
measures.define_measure(
    name="count_hi_opioid_wait",
    numerator=count_hi_opioid_wait,
...
# Prescribing post WL
measures.define_measure(
    name="count_hi_opioid_post",
    numerator=count_hi_opioid_post,
...
# Prescribing pre WL
measures.define_measure(
    name="count_hi_opioid_pre",
    numerator=count_hi_opioid_pre,
...
measures.define_measure(
    name="count_hi_opioid_wait_prior",
    numerator=count_hi_opioid_wait,
    denominator=denominator,
    intervals=weeks(52).starting_on("2000-01-01"),
