...
opioid_rx <- rbind(any_opioid_rx, hi_opioid_rx) %>%
    ...
    mutate(# Convert interval start date to number of weeks 
            ...
            dplyr::select(c(week, prior_opioid_rx, opioid_rx, denominator, rate,
                            rate_lci, rate_uci,
                           # opioid_rx_dummy, denominator_dummy, rate_dummy,
                            opioid_type, period)) %>%
...
ggplot(subset(opioid_rx, period %in% c("Pre-WL", "During WL") 
  ...
  geom_ribbon(aes(ymin = rate_lci, ymax = rate_uci, x = week), alpha = 0.2) +
...
ggplot(subset(opioid_rx, period %in% c("Pre-WL", "During WL") 
  ...
  geom_ribbon(aes(ymin = rate_lci, ymax = rate_uci, x = week, group = prior_opioid_rx), alpha = 0.2) +
  geom_vline(aes(xintercept = 0)) +
