...
full <- arrow::read_feather(here::here("output", "data", "dataset_full.arrow")) %>%
                ...
                mutate(sex_missing = ifelse(sex == "unknown", TRUE, FALSE),
                    sex_not_m_f = ifelse(!is.na(sex) & !(sex %in% c("unknown", "male", "female")),
                                         TRUE, FALSE),
                    start_before_end = ifelse((rtt_start_date < rtt_end_date) |
                                                  is.na(rtt_start_date), TRUE, FALSE),
                    wl_type_missing = ifelse(!(waiting_list_type %in% c("IRTT","ORTT","PTLO","PTLI","RTTO","RTTI")),
                                               TRUE, FALSE))
...
exclusions_1 <- full %>% 
  ...
  summarise(start_before_end = rounding(sum(start_before_end)),
            wl_type_missing = rounding(sum(wl_type_missing))) %>%
  ungroup() %>%
...
exclusions_2 <- full %>% 
  subset(start_before_end == FALSE & wl_type_missing == FALSE) %>%
  mutate(total = rounding(n())) %>%
  group_by(total) %>%
  summarise(sex_missing = rounding(sum(sex_missing)),
            sex_not_m_f = rounding(sum(sex_not_m_f))) %>%
  ungroup() %>%
  mutate(cohort = "2. Full WL population - no incorrect dates") %>%
  reshape2::melt(id = c("cohort", "total")) 
...
exclusions_6 <- ortho %>%
  ...
  ungroup() %>%
  mutate(cohort = "6. Orthopaedic - routine/admitted only") %>%
  reshape2::melt(id = c("total", "cohort"))
  
exclusions_7 <- ortho %>%
  subset(age_not_18_110 == FALSE & routine == "Routine" & admitted == TRUE & died_during_wl == FALSE) %>%
  mutate(total = rounding(n())) %>%
  group_by(total) %>%
  summarise(cancer = rounding(sum(cancer))) %>%
  ungroup() %>%
  mutate(cohort = "7. Orthopaedic - routine/admitted and alive at end") %>%
  reshape2::melt(id = c("total", "cohort"))
...
exclusions_8 <- ortho %>%
  ...
  ungroup() %>%
  mutate(cohort = "8. Orthopaedic - final") %>%
  reshape2::melt(id = c("total", "cohort"))
