...
# and checks for incorrect values
...
check_dates <- clockstops %>%
  ...
  mutate(rtt_end_before_start = ifelse(rtt_end_date < rtt_start_date, 1, 0),
...
# Save plot data 
write.csv(rtt_month, file = here::here("output", "clockstops", "plot_rtt_dates.csv"),
          row.names = FALSE)
...
write.csv(wait_time_pcent, here::here("output", "clockstops", "wait_time_percentiles.csv"),
          row.names = FALSE)

# By week
wait_time <- clockstops %>%
...
med_count_2 <- med_count %>%
  mutate(total_rx_round = rounding(total_rx),
         rate_pmonth = total_rx_round / person_days * 30) %>%
  dplyr::select(!total_rx)
